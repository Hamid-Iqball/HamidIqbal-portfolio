...
const NavItems = ()=>{
  return (
    <ul className=" flex flex-col py-2 items-center gap-8 sm:flex-row md:gap-6 relative z-20 bg-[#1B1B1B] border px-8 rounded-md ">
     {navLinks.map((link)=>(
      <Link href={link.href} key={link.id} className=" text-neutral-300 hover:text-white font-generalsans p-2  max-sm:hover:bg-black/50 max-sm:w-full max-sm:rounded-md py-2 max-sm:px-5;">{link.name}</Link>
     ))}
    </ul>
  )
}
